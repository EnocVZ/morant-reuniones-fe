...
export default {
    ...
    methods: {
        async _Temas() {
            let result = await temaReunionServices.Temas().then((res) => {
                this.Temas = res.data;
        ...
        async Guardar() {
            if (this.tema.id == 0) {
                ...
                let result = await temaReunionServices.updateTema(this.tema);
        ...
        async _eliminar() {
            let result = await temaReunionServices.deleteTema(this.idEliminar);
...
<template>
    <div>
        ...
        <p class="text-2xl mc-6">
            <VIcon icon="tabler-user" color="primary" class="me-3" size="30" />Lista de tema reunión
        </p>
        <VRow>
            <VCol cols="12" md="4">
          ...
          <VCard>
            ...
            <VCol cols="12" md="12">
                <VCard>
                    <VCardItem>
                        <VDataTable :headers="headers" :items="Temas" :search="search">
    ...
    <VDialog v-model="open" max-width="500px">
        <VCard>
            <VCardTitle>
                <span class="headline">{{
                    tema.id != 0 ? "Editar tema" : "Agregar tema"
                }}</span>
            </VCardTitle>
            <VCardText>
